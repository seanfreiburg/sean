...
Heli.Color = {
    BACKGROUND: '#35C4FC', BLOCK: '#63421B',
    HOME_TEXT: '#403B37', RAND_BLOCK: '#000000',
...
Heli.Screen = function (params) {
    ...
    img.src = 'style/img/bubble.png';
    img2.src = 'style/img/popped_bubble.png';
    ...
    function drawUser(ctx, user, trail, alternate, crashed) {
...
var HELICOPTER = (function () {
    ...
    function mainLoop() {
        ...
        if (state === Heli.State.PLAYING) {
            ...
            if (crashed) {
                screen.drawUser(ctx, pos, user.trail(), true, true);
            }
            else {
                screen.drawUser(ctx, pos, user.trail(), true, false);
            }
            ...
            screen.draw(ctx);
            screen.drawTerrain(ctx);
            screen.drawUser(ctx, pos, user.trail(), false, true);
